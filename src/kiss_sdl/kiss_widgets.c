...
int kiss_window_new(kiss_window *window, kiss_window *wdw, int decorate, int x,
                    int y, int w, int h) {
    if (!window)
        return -1;
...
int kiss_window_event(kiss_window *window, SDL_Event *event, int *draw) {
    ...
    if (event->type == SDL_WINDOWEVENT &&
        ...
        *draw = 1;
    if (!window->focus &&
        (!window->wdw || (window->wdw && !window->wdw->focus)))
        return 0;
...
int kiss_label_new(kiss_label *label, kiss_window *wdw, char *text, int x,
                   int y) {
    if (!label || !text)
        return -1;
...
int kiss_label_draw(kiss_label *label, SDL_Renderer *renderer) {
    ...
    for (p = label->text; *p; p = strchr(p, '\n') + 1) {
        ...
        kiss_rendertext(renderer, buf, label->rect.x, y, label->font,
                        label->textcolor);
...
int kiss_button_new(kiss_button *button, kiss_window *wdw, char *text, int x,
                    int y) {
    if (!button || !text)
        return -1;
...
int kiss_button_event(kiss_button *button, SDL_Event *event, int *draw) {
    ...
    if (event->type == SDL_WINDOWEVENT &&
        ...
        *draw = 1;
    if (!button->focus &&
        (!button->wdw || (button->wdw && !button->wdw->focus)))
        return 0;
    if (event->type == SDL_MOUSEBUTTONDOWN &&
...
int kiss_selectbutton_event(kiss_selectbutton *selectbutton, SDL_Event *event,
                            int *draw) {
...
int kiss_vscrollbar_new(kiss_vscrollbar *vscrollbar, kiss_window *wdw, int x,
                        int y, int h) {
    if (!vscrollbar)
        return -1;
    ...
    if (vscrollbar->up.h + vscrollbar->down.h + 2 * kiss_edge +
            2 * kiss_slider_padding + vscrollbar->vslider.h >
        h)
        return -1;
    kiss_makerect(&vscrollbar->uprect, x, y + kiss_edge, vscrollbar->up.w,
                  vscrollbar->up.h + kiss_slider_padding);
    ...
    kiss_makerect(&vscrollbar->sliderrect, x,
                  y + vscrollbar->uprect.h + kiss_edge, vscrollbar->vslider.w,
    ...
    vscrollbar->focus = 0;
    vscrollbar->min_y = vscrollbar->sliderrect.y;
    vscrollbar->max_y = vscrollbar->sliderrect.y + vscrollbar->maxpos;
    vscrollbar->wdw = wdw;
    return 0;
}
...
static void vnewpos(kiss_vscrollbar *vscrollbar, double step, int *draw) {
    *draw = 1;
    ...
    vscrollbar->sliderrect.y =
        vscrollbar->uprect.y + vscrollbar->uprect.h +
...
int kiss_vscrollbar_event(kiss_vscrollbar *vscrollbar, SDL_Event *event,
    ...
    if (!(SDL_GetMouseState(NULL, NULL) & SDL_BUTTON(SDL_BUTTON_LEFT))) {
        ...
        vscrollbar->downclicked = 0;
        vscrollbar->lasttick = 0;
    } else if (vscrollbar->upclicked &&
               kiss_getticks() > vscrollbar->lasttick + kiss_click_interval) {
    ...
    if (event->type == SDL_WINDOWEVENT &&
        ...
        *draw = 1;
    if (!vscrollbar->focus &&
        (!vscrollbar->wdw || (vscrollbar->wdw && !vscrollbar->wdw->focus)))
        return 0;
    if (event->type == SDL_MOUSEBUTTONDOWN &&
        kiss_pointinrect(event->button.x, event->button.y,
                         &vscrollbar->uprect) &&
        ...
        vscrollbar->sliderclicked = 0;
               ...
               ((event->motion.y > vscrollbar->min_y &&
                 event->motion.y < vscrollbar->max_y &&
                 event->motion.x >= vscrollbar->sliderrect.x &&
                 event->motion.x <=
                     vscrollbar->sliderrect.x + vscrollbar->sliderrect.w &&
                 vscrollbar->step > 0.000001) ||
                vscrollbar->sliderclicked)) {
        int mouse_y = event->motion.y - 7;

        if (mouse_y <= vscrollbar->min_y) {
            vscrollbar->fraction = 0;
        } else if (mouse_y >= vscrollbar->max_y) {
            vscrollbar->fraction = 1;
        } else {
            vscrollbar->fraction =
                (float)(mouse_y - vscrollbar->min_y) / vscrollbar->maxpos;
        }
...
int kiss_vscrollbar_draw(kiss_vscrollbar *vscrollbar, SDL_Renderer *renderer) {
    ...
    kiss_renderimage(renderer, vscrollbar->vslider, vscrollbar->sliderrect.x,
                     vscrollbar->sliderrect.y, NULL);
    return 1;
}

int kiss_hscrollbar_new(kiss_hscrollbar *hscrollbar, kiss_window *wdw, int x,
                        int y, int w) {
    if (!hscrollbar)
        return -1;
    ...
    if (hscrollbar->left.w + hscrollbar->right.w + 2 * kiss_edge +
            2 * kiss_slider_padding + hscrollbar->hslider.w >
        w)
        return -1;
    kiss_makerect(&hscrollbar->leftrect, x + kiss_edge, y,
                  hscrollbar->left.w + kiss_slider_padding, hscrollbar->left.h);
    kiss_makerect(&hscrollbar->rightrect,
                  x + w - hscrollbar->right.w - kiss_slider_padding - kiss_edge,
                  y, hscrollbar->right.w + kiss_slider_padding,
                  hscrollbar->right.h);
    kiss_makerect(&hscrollbar->sliderrect,
                  x + hscrollbar->leftrect.w + kiss_edge, y,
                  hscrollbar->hslider.w, hscrollbar->hslider.h);
    ...
    hscrollbar->focus = 0;
    hscrollbar->min_x = hscrollbar->sliderrect.x;
    hscrollbar->max_x = hscrollbar->sliderrect.x + hscrollbar->maxpos;
    hscrollbar->wdw = wdw;
    return 0;
}
...
static void hnewpos(kiss_hscrollbar *hscrollbar, double step, int *draw) {
    *draw = 1;
    ...
    hscrollbar->sliderrect.x =
        hscrollbar->leftrect.x + hscrollbar->leftrect.w +
...
int kiss_hscrollbar_event(kiss_hscrollbar *hscrollbar, SDL_Event *event,
    ...
    if (!(SDL_GetMouseState(NULL, NULL) & SDL_BUTTON(SDL_BUTTON_LEFT))) {
        ...
        hscrollbar->rightclicked = 0;
        hscrollbar->lasttick = 0;
    } else if (hscrollbar->leftclicked &&
               kiss_getticks() > hscrollbar->lasttick + kiss_click_interval) {
        hnewpos(hscrollbar, -hscrollbar->step, draw);
        return 1;
    } else if (hscrollbar->rightclicked &&
               kiss_getticks() > hscrollbar->lasttick + kiss_click_interval) {
        hnewpos(hscrollbar, hscrollbar->step, draw);
        return 1;
    }
    ...
    if (event->type == SDL_WINDOWEVENT &&
        ...
        *draw = 1;
    if (!hscrollbar->focus &&
        (!hscrollbar->wdw || (hscrollbar->wdw && !hscrollbar->wdw->focus)))
        return 0;
    if (event->type == SDL_MOUSEBUTTONDOWN &&
        kiss_pointinrect(event->button.x, event->button.y,
                         &hscrollbar->leftrect)) {
        ...
        hscrollbar->sliderclicked = 0;
               ...
               (event->motion.state & SDL_BUTTON(SDL_BUTTON_LEFT)) &&
               ((event->motion.x > hscrollbar->min_x &&
                 event->motion.x < hscrollbar->max_x &&
                 event->motion.y >= hscrollbar->sliderrect.y &&
                 event->motion.y <=
                     hscrollbar->sliderrect.y + hscrollbar->sliderrect.h &&
                 hscrollbar->step > 0.000001) ||
                hscrollbar->sliderclicked)) {
        int mouse_x = event->motion.x - 7;

        if (mouse_x <= hscrollbar->min_x) {
            hscrollbar->fraction = 0;
        } else if (mouse_x >= hscrollbar->max_x) {
            hscrollbar->fraction = 1;
        } else {
            hscrollbar->fraction =
                (float)(mouse_x - hscrollbar->min_x) / hscrollbar->maxpos;
        }
...
int kiss_progressbar_new(kiss_progressbar *progressbar, kiss_window *wdw, int x,
                         int y, int w) {
...
int kiss_progressbar_event(kiss_progressbar *progressbar, SDL_Event *event,
    ...
    if (!progressbar || !progressbar->visible)
        return 0;
    if (progressbar->run &&
        kiss_getticks() > progressbar->lasttick + kiss_progress_interval) {
...
int kiss_progressbar_draw(kiss_progressbar *progressbar,
    ...
    progressbar->barrect.w =
        (int)(progressbar->width * progressbar->fraction + 0.5);
...
int kiss_entry_new(kiss_entry *entry, kiss_window *wdw, int decorate,
    ...
    kiss_string_copy(entry->text,
                     kiss_maxlength(entry->font, entry->textwidth, text, NULL),
                     text, NULL);
    kiss_makerect(&entry->rect, x, y, w,
                  entry->font.fontheight + 2 * kiss_border);
...
int kiss_entry_event(kiss_entry *entry, SDL_Event *event, int *draw) {
    ...
    if (event->type == SDL_MOUSEBUTTONDOWN && !entry->active &&
        ...
        *draw = 1;
        ...
        if (kiss_textwidth(entry->font, entry->text, event->text.text) <
                entry->textwidth &&
...
int kiss_textbox_event(kiss_textbox *textbox, SDL_Event *event, int *draw) {
    ...
    if (event->type == SDL_WINDOWEVENT &&
        ...
        *draw = 1;
    if (!textbox->focus &&
        (!textbox->wdw || (textbox->wdw && !textbox->wdw->focus)))
        return 0;
    if (event->type == SDL_MOUSEBUTTONDOWN &&
        kiss_pointinrect(event->button.x, event->button.y,
                         &textbox->textrect)) {
...
int kiss_textbox_draw(kiss_textbox *textbox, SDL_Renderer *renderer) {
    ...
    for (i = 0; i < numoflines; i++) {
        kiss_string_copy(
            buf,
            kiss_maxlength(
                textbox->font, textbox->textwidth,
                (char *)kiss_array_data(textbox->array, textbox->firstline + i),
                NULL),
            (char *)kiss_array_data(textbox->array, textbox->firstline + i),
            NULL);
        kiss_rendertext(renderer, buf, textbox->textrect.x,
                        textbox->textrect.y + i * textbox->font.lineheight +
                            textbox->font.spacing / 2,
                        textbox->font, textbox->textcolor);
    }
    return 1;
}

int kiss_combobox_new(kiss_combobox *combobox, kiss_window *wdw, char *text,
                      kiss_array *a, int x, int y, int w, int h) {
    if (!combobox || !a || !text)
        return -1;
    ...
    if (combobox->textbox.array->length - combobox->textbox.maxlines > 0)
        combobox->vscrollbar.step =
            1. / (combobox->textbox.array->length - combobox->textbox.maxlines);
    return 0;
}
...
int kiss_combobox_event(kiss_combobox *combobox, SDL_Event *event, int *draw) {
    ...
    if (kiss_vscrollbar_event(&combobox->vscrollbar, event, draw) &&
        ...
        combobox->vscrollbar.step = 0.;
        if (combobox->textbox.array->length - combobox->textbox.maxlines > 0)
            combobox->vscrollbar.step = 1. / (combobox->textbox.array->length -
                                              combobox->textbox.maxlines);
        firstline = (int)((combobox->textbox.array->length -
                           combobox->textbox.maxlines) *
                              combobox->vscrollbar.fraction +
                          0.5);
        if (firstline >= 0)
    ...
    if (kiss_entry_event(&combobox->entry, event, draw)) {
        ...
        *draw = 1;
        ...
        kiss_string_copy(
            combobox->entry.text,
            kiss_maxlength(
                combobox->textbox.font, combobox->entry.textwidth,
